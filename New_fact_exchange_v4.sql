...
,main AS (
    SELECT
		...
		convert(date,TARIKH_MSHLOCH,112) [old_date]
        -- ,bb.MS_HZMNH
...
--	b.UnitNetPriceUSD,
--	a.old_date,
--	row_number() over ( order by a.Date desc)
--FROM main a
--LEFT JOIN (
--    SELECT *
--    FROM totals
...
--WHERE 1 = 1
---- AND DeliveredFrom = 146 OR DeliveredTo = 146
----AND (a.ItemKey = '40-40' or b.ItemKey = '40-40')

--ORDER BY a.old_date DESC
, base as (
select 
a.DeliveredFrom,
a.DeliveredFromName,
...
a.Date,
b.UnitNetPriceUSD,
dense_rank() over (order by a.date desc) as grp
FROM main a
LEFT JOIN (
    SELECT *
    FROM totals
...
*,
case 
	when UnitNetPriceUSD is null then LAST_VALUE(UnitNetPriceUSD)  OVER (PARTITION BY grp ORDER BY Date desc)
	else UnitNetPriceUSD end as test2
from base
ORDER BY Date DESC
